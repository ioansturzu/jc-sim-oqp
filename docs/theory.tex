...
In this report, we adopt an \emph{Operational} perspective. We do not merely solve the Schrödinger equation; we treat the system as an open quantum system subject to continuous observation (instruments). This aligns the theory with the software architecture of \texttt{jc-sim-oqp}, where "Solvers" correspond to different physical unravellings of the dynamics.
...
We perform the \emph{Rotating Wave Approximation (RWA)} to neglect these terms, yielding:
\begin{equation}
  \hat{H}_{I,\text{RWA}} = \hbar g \left(\hcd \sigmam + \hc \sigmap\right).
\end{equation}

Transforming back to the Schr\"odinger picture, the resulting \emph{Jaynes--Cummings Hamiltonian} is:
\begin{equation}
  \boxed{
...
The eigenvectors, known as \emph{dressed states}, are:
\begin{align}
...
\end{equation}
This coherent oscillation at frequency $2g$ is the \emph{Vacuum Rabi Oscillation}.
...
1.  \emph{Collapse:} The oscillations dephase on a timescale $t_c \sim 1/g$.
2.  \emph{Revival:} The phases realign at $t_{rev} \sim \frac{2\pi\sqrt{\bar{n}}}{g}$.
...
While standard texts often postulate the Master Equation, we derive it here from microscopic principles, following the \emph{Reservoir Model} formalism detailed in the main OQP Report.
...
\end{equation}
In the interaction picture, using the \emph{Born Approximation} (weak coupling, $\rho_{tot} \approx \rho \otimes \rho_B$) and the \emph{Markov Approximation} (bath memory time $\tau_B \to 0$ or white noise spectrum), we trace over the bath.

\subsection{The Lindblad Master Equation}
This leads to the \emph{Lindblad Master Equation} for the reduced density matrix $\rho$:
\begin{equation}
...
The Purcell effect, first predicted by Edward Purcell in 1946, describes the enhancement of a quantum emitter's spontaneous emission rate when it is placed inside a resonant cavity. In the \texttt{jc-sim-oqp} package, this is treated as a core dissipative mechanism.

When an atom is coupled to a cavity mode with total decay rate $\kappa$ and coupling strength $g$, the emitter experiences an enhanced decay into the cavity mode. In the \emph{bad-cavity limit} ($\kappa \gg g, \gamma$), we can adiabatically eliminate the cavity mode to find an effective decay rate for the atom.

\subsection{Purcell Rate and Factor}
The \emph{Purcell rate}, $\gamma_P$, represents the rate of emission into the cavity mode:
\begin{equation}
    \gamma_P = \frac{4g^2}{\kappa}
\end{equation}
The total decay rate of the atom is then $\gamma_{tot} = \gamma + \gamma_P$, where $\gamma$ is the free-space decay rate. We define the \emph{Purcell Factor} ($F_p$) as the ratio of the enhanced rate to the free-space rate:
\begin{equation}
    F_p = \frac{\gamma_P}{\gamma} = \frac{4g^2}{\kappa \gamma}
\end{equation}

\subsection{Cooperativity and Beta Factor}
A related dimensionless parameter is the \emph{cooperativity} $\eta$, which quantifies the ratio of coherent coupling to dissipative rates. Including pure dephasing $\gamma_\phi$, the cooperativity is implemented as:
\begin{equation}
    \eta = \frac{2g^2}{\kappa (\gamma/2 + \gamma_\phi)}
\end{equation}
The fraction of the total emission that goes into the cavity mode, known as the \emph{beta factor} ($\beta$), is given by:
\begin{equation}
    \beta = \frac{\gamma_P}{\gamma + \gamma_P} = \frac{F_p}{1 + F_p}
\end{equation}
High $\beta$ factors indicate that the cavity mode dominates the emitter's dissipative environment, a prerequisite for efficient single-photon sources and quantum interconnects.
...
We rely on the \emph{Minimal Dilation Theorem} (Maassen, Sz.-Nagy), a core result of OQP.
...
This theoretical insight leads directly to the \emph{Stochastic Solver}.
We continuously monitor the bath. This yields a measurement record $N(t)$ (e.g. photon clicks).
The conditional state $\ket{\psi_c(t)}$ evolves under the \emph{Stochastic Schrödinger Equation}:
1.  \emph{Drift:} Between jumps, evolution is governed by a non-Hermitian Hamiltonian:
    \begin{equation}
        H_{eff} = H - \frac{i\hbar}{2}\sum_k L_k^\dagger L_k.
    \end{equation}
    (This causes the norm to decay, reflecting the "waiting time" distribution).
2.  \emph{Jump:} When a jump occurs (probability $\delta p = \bra{\psi}L^\dagger L \ket{\psi}dt$), the state collapses:
    \begin{equation}
...
1.  \emph{AC Stark Shift:} The qubit frequency shifts by $2\chi n$.
2.  \emph{Cavity Pull:} The cavity frequency shifts by $\pm \chi$ depending on the qubit state $\sigma_z$.

This is the standard regime for \emph{Circuit QED Readout}.

\section{Quantum Instruments and Tomography}
\label{sec:instruments}

\subsection{Measurement as an Instrument}
In OQP, a measurement is defined by a \emph{Quantum Instrument} $\mathcal{I}$.
...
Realistic measurements are never ideal projectors. As derived in the Report's Stern-Gerlach analysis, physical noise (gradient fluctuations, amplifier noise) leads to \emph{Fuzzification}.
The observable is derived from a \emph{Positive Operator-Valued Measure (POVM)}:
\begin{equation}
...
We consider a neutral particle with spin 1/2 and mass $m$ moving in a magnetic field $\vec{B}$. Unlike the impulsive approximation used in OQP (where kinetic energy is neglected), here we solve the full \emph{Pauli-Schrödinger equation}:
\begin{equation}
...
The ideal separation derived above assumes a perfectly isolated system. OQP Chapter 7 explicitly models the environment using the \emph{Quantum Noise} formalism.
...
\end{equation}
In the Markovian limit, this interaction leads to a \emph{Quantum Dynamical Semigroup} generator $\mathcal{L}$ of the \emph{Lindblad form}, physically corresponding to momentum diffusion. This diffusion broadens the wave packets in momentum space, creating an overlap region where the spin components are not perfectly distinguishable.
...
\begin{itemize}
    \item \emph{Parameters:} $\omega_c \sim 300$ THz, $g \sim 10$ MHz.
    \item \emph{Regime:} Strong coupling is achievable ($g \gg \kappa, \gamma$).
    \item \emph{Relevance:} Foundation for quantum networks and distributed quantum computing.
\end{itemize}

\subsection{Superconducting Circuit QED}
In circuit QED, Transmon qubits couple to coplanar waveguide resonators.
\begin{itemize}
    \item \emph{Parameters:} $\omega_c \sim 5$ GHz, $g \sim 100$ MHz.
    \item \emph{Regime:} Can reach Ultrastrong coupling.
    \item \emph{Features:} Lithographically engineerable. Dispersive readout is the standard.
\end{itemize}
