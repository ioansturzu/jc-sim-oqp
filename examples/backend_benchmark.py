...
def _run_scqubits_path(params: SimParams) -> SimResult:
    """Build Hamiltonian via scqubits, solve via QuTiP mesolve.

    Maps SimParams to TransmonCavityParams by treating ωa as the
    target transmon ω₀₁.  Uses EJ/EC ≈ 50 (typical transmon ratio).
    """
    ...
    a, sm = qutip_be.build_operators(params.N, params.n_atoms)
    psi0 = qutip_be.build_initial_state(params.N, params.n_atoms)
    c_ops = qutip_be.build_collapse_ops(params, a, sm)

    n_cav = a.dag() * a
    n_atom = sum(s.dag() * s for s in sm)
    e_ops = [n_cav, n_atom]

    return qutip_be.mesolve(H, psi0, params.tlist, c_ops, e_ops)
...
def run_benchmark(
    ...
    for regime_name in regimes:
        ...
        for n_cav in sizes:
            ...
            # scqubits+QuTiP (only for single-atom configs)
            if params.n_atoms == 1:
                try:
                    tracemalloc.start()
                    res_sc = _run_scqubits_path(params)
                    _, peak_sc = tracemalloc.get_traced_memory()
                    tracemalloc.stop()

                    results.append(BenchmarkResult(
                        backend="scqubits+qutip", regime=regime_name,
                        n_cavity=n_cav, hilbert_dim=hdim,
                        wall_time=res_sc.wall_time,
                        peak_memory_kb=peak_sc / 1024,
                        l2_error=None,
                    ))
                    print(f"  scqubits+qutip: {res_sc.wall_time:.4f}s")
                except Exception as exc:  # noqa: BLE001
                    print(f"  scqubits+qutip: FAILED ({exc})")
